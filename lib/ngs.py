...
def annotate_peaks_RNA(data, annotation, useStr=False):
    for e in data:
        ...
        for t in annotation.get(chr, []):
            ...
            cds_a, cds_b = t[6], t[7]
            if useStr and not (k >= tr_a and k <= tr_b and e_str == strand):
                continue
...
def RNA_peaks_annotate_frombed(peaksFile, outputFile, annotationFile, header_sup, useStr=False):
    header_sup = header_sup.split(",")
    annotation = read_genomic_annotation(annotationFile)
    peaks      = read_peaks_bed(peaksFile)
    peaks      = annotate_peaks_RNA(peaks, annotation, useStr)
